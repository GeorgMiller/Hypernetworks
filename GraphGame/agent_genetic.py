...
class Agent():
    ...
    def train_generation(self):
        ...
        if self.step % 100 == 0:
            self.n_iter += 1
        self.epsilons = (self.sigma**2) * np.random.normal(0,1,(self.eps,self.weight_space))
        self.epsilons[9,:] = np.zeros(self.weight_space)
        self.R = []
        embedding_matrix = []
        ...
        for idx in range(self.population_size):
            ...
            for e in self.epsilons:
                ...
                agents_weights = self.agents[idx] + e
                self.set_weights(agents_weights)
                reward, embedding = self.evaluate()
                self.R.append(reward/self.evaluate_num)
                embedding_matrix.append(embedding)

        embedding_matrix = np.reshape(embedding_matrix,(self.population_size,len(self.epsilons),self.batch_size))
        mean = np.mean(self.R)
        self.R = np.reshape(self.R,(self.population_size, self.eps))

        
        if self.step >= 3:

            self.calculate_dvd(embedding_matrix)
            ...
            for idx in range(self.population_size):
                ...
                for i, e in enumerate(self.epsilons):
                    ...
                    rewards = np.mean((1-self.lamBda)*self.R[idx, i]) 
                    gradient = (rewards + self.lamBda * self.dvd[i])*e
                    update = self.Adam(gradient, idx, self.learning_rate, self.n_iter)
                    #change = self.learning_rate * (np.mean((1-self.lamBda)*self.R[i]) + self.lamBda * self.dvd[i])*e

                    self.agents[idx] = self.agents[idx] + update #gradient * self.learning_rate
            ...
            print(self.R, self.R[:,i], self.R[idx,i])
        ...
        for idx in range(len(self.epsilons)):
            ...
            for i in range(self.population_size):
                for j in range(self.population_size): 
                    z = tf.exp(-tf.math.squared_difference(embedding_matrix[i,idx],embedding_matrix[j,idx]))
            ...
            self.dvd.append(determinant.numpy())
    ...
    def evaluate(self):
        ...
        return np.mean(total_score), np.reshape(embeddings,-1)
